...
def main():
    ...
    writer = SummaryWriter('runs/DPO-test-reward-{}-{}'.format(args.env_name, args.seed))
    ...
    memory = ReplayBuffer(envs.observation_space.shape, envs.action_space, device)
    ...
    for j in tqdm(range(num_updates)):            
        ...
        for step in range(args.num_steps):
            ...
            truncated = torch.as_tensor(done and
                                        [info.get("terminal_observation") is not None for info in infos] and
        ...
        with torch.no_grad():
            ...
            rollouts.qvalues[:-1].copy_(q.unsqueeze(1))
            rollouts.baselines.copy_(b.view(*b.shape, 1, 1))
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1
            ...
            avg_reward = evaluator.eval(obs_rms)
            total_num_steps = (j + 1) * args.num_processes * args.num_steps
